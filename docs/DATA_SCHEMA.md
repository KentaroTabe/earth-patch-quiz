...
`wikipedia` と `reject` と `category` は仕様 §6.1 に無い項目。知名度の実測、不採用の理由、分野の偏りを見るために足した。

### 分野（`category`）

10種類。**特定の分野に偏らないよう、足すときは少ない分野から埋める。**

`火山` / `湖` / `河川` / `海岸` / `砂漠` / `氷河` / `山地` / `都市` / `農地` / `広大`

`広大` は「特徴が少なく非常に広い場所」。ルブアルハリ砂漠やタネズルフトのように、
手がかりがほとんど無いことそのものが問題になっているもの。機械の下見は
「のっぺりしている」として落とそうとするので、そこは意図して残す。

`npm run sheet` は分野ごとにまとめて並べるので、どこが薄いか一目で分かる。
